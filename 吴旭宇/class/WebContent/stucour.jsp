			...
			<table class="table table-striped table-bordered">
				...
				<c:forEach items="${courses }" var="course" varStatus="courses">
					<tr>
						...
						<td>${course.point }</td>
						<td>
							<button class="btn btn-primary btnSub" name="id" data-id="${course.id }">提交选课</button>
						</td>
					</tr>
				</c:forEach>
...
<!-- 			分页标签 -->
			<div class="text-right">
				<ul class="pagination">
					<c:choose>
						<c:when test="${cp eq 1 }">
							<li class="disabled"><a href="stucour?method=findAll&cp=${cp-1 }"><span>&laquo;</span></a></li>
						</c:when>
						<c:otherwise>
							<li><a href="stucour?method=findAll&cp=${cp-1 }"><span>&laquo;</span></a></li>
						</c:otherwise>
					</c:choose>
					...
					<c:forEach begin="1" end="${tp }" var="page">
						<c:choose>
							...
							<c:otherwise>
								<li class=""><a href="stucour?method=findAll&cp=${page }">${page }</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
...
<!-- 					<li><a href="">3</a></li> -->
					<c:choose>
						<c:when test="${cp eq tp }">
							<li class="disabled"><a href="stucour?method=findAll&cp=${cp+1 }"><span>&raquo;</span></a></li>
						</c:when>
						<c:otherwise>
							<li><a href="stucour?method=findAll&cp=${cp+1 }"><span>&raquo;</span></a></li>
						</c:otherwise>
					</c:choose>
				</ul>
			</div>
	...
	$(function(){
		...
		$('.btnSub').click(function(){
			...
			layer.confirm('确认提交？', {
				  ...
				  location.href='stucour?method=subCours&id='+id;
				});
		})
